...
import { MessageSquare, GripVertical, Plus, Trash2, AlertCircle } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 space-y-3 min-w-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="space-y-2">
            <Label htmlFor={fieldControlId(fieldId, 'label')} className="text-foreground">Label</Label>
            <Input
              id={fieldControlId(fieldId, 'label')}
              value={field.label}
              onChange={(e) => onUpdate({ label: e.target.value })}
              placeholder="Field label"
              className="bg-primary-700 text-foreground rounded p-3 placeholder:text-muted-foreground focus:ring-2 focus:ring-accent-500"
              aria-label="Field label"
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor={fieldControlId(fieldId, 'type')} className="text-foreground">Type</Label>
            <Select
              value={field.type}
            ...
            >
              <SelectTrigger
                ...
                className="bg-primary-700 text-foreground rounded p-3 focus:ring-2 focus:ring-accent-500"
        ...
        <div className="flex flex-wrap items-center gap-4">
          <div className="flex-1 min-w-[200px] space-y-2">
            <Label htmlFor={fieldControlId(fieldId, 'placeholder')} className="text-foreground">Placeholder</Label>
            <Input
          ...
          <div className="flex-1 min-w-[140px] space-y-2">
            <Label htmlFor={fieldControlId(fieldId, 'sampleData')} className="text-foreground">Sample data</Label>
            <Input
          ...
          <div className="flex items-center gap-2 pt-6">
            ...
            />
            <Label htmlFor={fieldControlId(fieldId, 'required')} className="cursor-pointer text-foreground text-lg font-medium">
              Required
            </Label>
          </div>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          <Label className="w-full text-foreground">Validation rules</Label>
            ...
            return (
              <div key={rule.value} className="flex items-center gap-2">
                ...
                <Input
                  ...
                  className="w-24 bg-primary-700 text-foreground rounded p-3 placeholder:text-muted-foreground focus:ring-2 focus:ring-accent-500"
      ...
      <Button
        ...
        size="icon"
        onClick={onRemove}
        className="text-destructive hover:text-destructive shrink-0 min-h-[44px] min-w-[44px] touch-manipulation"
  ...
  return (
    ...
    >
      {[1, 2, 3].map((i) => (
        ...
        >
          <Skeleton className="h-5 w-5 mt-2 rounded shrink-0 skeleton-shimmer" />
          <div className="flex-1 space-y-3 min-w-0">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Skeleton className="h-10 w-10 rounded shrink-0 skeleton-shimmer" />
        </div>
      ))}
      <span className="sr-only">Loading form fields, please wait.</span>
    </div>
  )
}
...
export function FieldsEditor({
  ...
  return (
    <Card
      className="bg-secondary-800 p-6 rounded-lg shadow-card overflow-hidden transition-all duration-300 hover:shadow-card-hover border-divider"
      aria-labelledby="fields-editor-title"
      aria-describedby="fields-editor-description"
    >
      <CardHeader className="flex flex-row items-center justify-between p-0 pb-4">
        <div>
          <CardTitle
            id="fields-editor-title"
            className="flex items-center gap-2 text-foreground"
          >
            <MessageSquare className="h-5 w-5 text-accent-500" aria-hidden />
            Fields
          </CardTitle>
          <CardDescription id="fields-editor-description" className="text-muted-foreground">
            Add, remove, and reorder fields to collect
          </CardDescription>
        </div>
        <Button
          size="sm"
          onClick={onAddField}
          className="bg-accent-500 text-accent-foreground hover:bg-accent-600 transition-all duration-200 hover:scale-[1.02] hover:shadow-glow"
      ...
      <CardContent className="space-y-4">
        {fieldError && (
          ...
          >
            ...
            <p id="fields-error-message">{fieldError}</p>
          </div>
        )}
        <DndContext
          sensors={sensors}
          ...
          accessibility={{
            announcements: {
              onDragStart: () => 'Picked up a form field. Use arrow keys to reorder.',
              onDragOver: () => 'Form field moved to new position.',
              onDragEnd: () => 'Form field dropped. Reorder complete.',
              onDragCancel: () => 'Reordering cancelled. Form field returned to original position.',
            },
          }}
        >
          <SortableContext
          ...
          >
            <div className="space-y-4" aria-label="Form fields list">
              {isLoading ? (
                ...
                <div
                  className={cn(
                    ...
                    'border-divider bg-muted/30',
                    fieldError && 'border-destructive/50 bg-destructive/5'
                  )}
                  role="status"
                  aria-label="No form fields added yet. Add your first field to get started."
                >
                  <div className="icon-empty-state w-16 h-16 text-muted-foreground mx-auto mb-4">
                    <MessageSquare className="h-16 w-16" aria-hidden />
                  </div>
                  <h2 className="text-2xl font-semibold text-center text-foreground">No fields yet</h2>
                  <p className="text-center text-muted-foreground mt-2 max-w-sm mx-auto">
                    Add fields to collect information conversationally. Drag to reorder once added.
                  </p>
                  <Button
                    size="lg"
                    className="bg-accent-500 text-accent-foreground rounded-lg px-6 py-2 hover:bg-accent-600 transition-all duration-200 hover:scale-[1.02] hover:shadow-glow mt-6 gap-2 min-h-11"
                    onClick={onAddField}
